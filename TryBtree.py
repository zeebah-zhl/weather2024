...
def main():
	...
	B = BTree(50)
	print('==========Loading==========')
	with open('C:/Users/ZJ/Downloads/B-TreeImp/Fivethousand.txt', 'r') as f:
		for line in f:
			word = line.split("\t")
			#B.insert((float(word[0]), word[1]))
			B.insert((float(word[0]), word[1].strip()))
	print('==========Loaded==========')
	
	c = 5
	while c > 0 :
		print("Enter your choice")
		print("1) Insert ra and dec")
		print("2) Delete")
		print("3) Search using ra or dec")
		print("4) Print the B-Tree")
		print("0) Exit")
		c = int(input())

		if c == 1:
			ra = float(input("Enter the ra value: "))
			dec = float(input("Enter the dec value : "))
			#B.insert((float(ra), float(dec)))
			B.insert((ra,dec))
			print("Your data has been entered")
		elif c == 2:
			ra_dec_value = input("Enter the the value: ")
			B.delete(B.root, float(ra_dec_value))
		elif c == 3:
			coordinate = float(input("Enter the value to be searched: "))
			start = time.perf_counter()
			#begin = time.time()
			K = B.search(coordinate)
			if K != None:
				(x, i) = K
				print("The ra value ", x.keys[i][0], " has following dec value", x.keys[i][1])
				#time.sleep(1)
				#end = time.time()
				#print(f"Total search runtime is {end - begin}")
			
			else:	
				print("The value does not exist!")
			time_btree = time.perf_counter() - start
			print("Time taken to search: ", time_btree)
		elif c == 4:
			B.print_tree(B.root)		
		else :
			break
